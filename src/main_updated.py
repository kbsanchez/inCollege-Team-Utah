...
def create_table():
    query = """CREATE TABLE IF NOT EXISTS Username(username TEXT, password TEXT,fn TEXT,ln TEXT)"""
    c.execute(query)
    conn.commit()
...
def find_in_db(fn,ln):
    
    for row in c.execute("""SELECT * FROM Username"""):
        if fn == row[2] and ln == row[3]:
...
def main():
    ...
    while choice != 'q':
        ...
        if choice=='f':
            ...
            find_in_db(fn,ln)
            rn=input("please enter to the previous page")
        if choice=='s':
            print("video is now playing")
            rn=input("press enter to the previous page")
    ...
    #CREATES NEW ACCOUNT
            ...
            if capacity < 5:
                ...
                password = getpass()
                check_pw(password)
                data_entry(username2, password,fn,ln)
            elif capacity == 5:
